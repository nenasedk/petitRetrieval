...
# Data Setup
RETRIEVAL_NAME = '2M1207B_v2'
INPUT_DIR = '/home/ipa/quanz/user_accounts/evertn/SimulatedObs/BrownDwarf_v3/jwst_spectra/' # end with forward slash!
...
OBJ_NAME = '2M1207'
IS_COMPANION = False # False if brown dwarf
WLEN = [4.9, 18.1]
LOG_G = 3.20
R_pl =   1.5*nc.r_jup_mean
...
LINE_SPECIES =  ['CO_all_iso','H2O','CH4','NH3','CO2','HCN','TiO','C2H2']
...
PRIORS['t_int']          = lambda x: uniform_prior(x, 0., 2500.)# probably should be increased
PRIORS['t_equ']          = lambda x: uniform_prior(x, 0., 3000.)
...
LOG_PRIORS['t_int']          = lambda x: a_b_range(x, 0., 2500.) 
LOG_PRIORS['t_equ']          = lambda x: a_b_range(x, 0., 3000.)
...
for line in LINE_SPECIES:
    LOG_PRIORS[line] = lambda x: a_b_range(x, -18., 0.)
    PRIORS[line] = lambda x: uniform_prior(x, -18., 0.)
if len(CLOUD_SPECIES) > 0:
    for cloud in CLOUD_SPECIES:
        LOG_PRIORS[cloud] = lambda x: a_b_range(x, -18., 0.)
        PRIORS[cloud] = lambda x: uniform_prior(x, -18., 0.)
